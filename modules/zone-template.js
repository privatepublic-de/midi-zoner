...
module.exports = {
  ...
  getHTML: function (/** @type {Zone} */ zone, zoneindex) {
    ...
    for (let i = 0; i < Zone.Sequence.MAX_STEPS; i++) {
      sequencerGrid += `<div class="step" data-action="${index}:select_step:${i}" data-contextmenu="${index}:seq_copy_step:${i},${index}:seq_paste_step:${i},-,${index}:seq_copy,${index}:seq_paste,${index}:seq_paste_steps:${i},-,${index}:seq_clear_step:${i},${index}:seq_clear_all">${
        i + 1
      }</div>`;
    }
